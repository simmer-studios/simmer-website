...
const ServiceCategories: FC<
  HTMLProps<HTMLDivElement> & ServiceCategoriesProps
> = ({ categories, selectedCategory, onCategoryClick, ...props }) => {
  return (
    <section className="py-10 lg:py-16" {...props}>
...
const Row: FC<RowProps> = ({
  ...
  return (
    <div className={cn("block w-full", className)}>
      ...
      >
        {active && (
          <span className="absolute right-2 top-0 inline-block aspect-square h-[20px] translate-y-4 rounded-full bg-simmer-yellow sm:left-2 md:left-5 md:h-[25px] lg:top-4" />
        )}
        {/* numbering */}
